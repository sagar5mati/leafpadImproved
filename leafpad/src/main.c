...
#define GLOBAL_VARIABLE_DEFINE				//used for #ifdef in leafpad.h
#include "leafpad.h"
#if GLIB_CHECK_VERSION(2, 6, 0)				//Standard Macro for checking glib version
...
#	include <getopt.h>				//why include getopt.h instead?
#endif
//conf, stores user changes made to the editor - configuration
typedef struct {
	gint width;
	gint height;
	gchar *fontname;
...
//Loads config file from some path
static void load_config_file(Conf *conf)
{
...
#if GLIB_CHECK_VERSION(2, 6, 0)
	path = g_build_filename(g_get_user_config_dir(),		//builds pathname from the path obtained from g-get-user-blah
	    PACKAGE, PACKAGE "rc", NULL);
									//path is ~/.config/leafpad
#else
	...
	g_free(path);
	if (!fp)
		return;					//gchar **strsplit()

	if (fgets(buf, sizeof(buf), fp)) {		//char *fgets(char *s, int size, FILE *stream);
		num = g_strsplit(buf, "." , 3);		//BUFSIZ is macro defined in glibc ??
		if ((atoi(num[1]) >= 8) && (atoi(num[2]) >= 0)) {	//fgets reads upto size, newline or EOF
			fgets(buf, sizeof(buf), fp);			//g_strsplit tokenizer
			conf->width = atoi(buf);			//some condition if true get the conf
...
#if !GLIB_CHECK_VERSION(2, 6, 0)				//help menu for older version, struct definition ??
